...
def digit_distance(num: int) -> int:
    ...
    last_digit, remaining_digits = num % 10, num // 10

    return abs(last_digit - (remaining_digits % 10)) + digit_distance(remaining_digits)
...
def interleaved_sum(num: int, f_odd, f_even) -> int:
    ...
    def odd_sum(n):
        ...
        return f_odd(n) + f_even(n + 1) + odd_sum(n + 2)

    # mutal recursion solution
    # def odd_sum(n):

    #     if n > num:
    #         return 0

    #     return f_odd(n) + even_sum(n + 1)
...
def count_dollars(sum_needed: int) -> int:
    ...
    def helper(amt, bill):
        ...
        return helper(amt - bill, bill) + helper(amt, next_smaller_dollar(bill))
...
def shuffle(s: list) -> list:
    ...
    return [x for pair in zip(s[:half], s[half:]) for x in pair]
...
def count_dollars_upward(sum_needed: int) -> int:
    ...
    def helper(amt, bill):
        ...
        return helper(amt - bill, bill) + helper(amt, next_larger_dollar(bill))
