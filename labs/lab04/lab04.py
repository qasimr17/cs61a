...
def closer_city(lat, lon, city_a, city_b):
    ...
    'Bucharest'
    """
    base_city = make_city("base_city", lat, lon)

    return (
        get_name(city_a)
        if distance(city_a, base_city) < distance(city_b, base_city)
...
def sum_tree(t):
    ...
    15
    """
    # can do without this condition as well since leaf node has branches = [] and sum([]) = 0
    if is_leaf(t):
        return label(t)
...
def balanced(t):
    ...
    return len(set(sum_tree(b) for b in branches(t))) == 1 and all(
        balanced(b) for b in branches(t)
    )
