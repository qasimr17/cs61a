...
class Transaction:
    ...
    def report(self) -> str:
        ...
        msg: str = "no change"
        if self.changed():
            change = "increased" if self.after > self.before else "decreased"
            msg = f"{change} {self.before}->{self.after}"
...
class BankAccount:
    ...
    def deposit(self, amount: int) -> int:
        ...
        transaction = Transaction(id=self._next_id(), before=before, after=self.balance)
    ...
    def withdraw(self, amount: int) -> int | str:
        ...
        if amount > self.balance:
            # Create transaction with no change to balance
            transaction = Transaction(id=self._next_id(), before=before, after=before)
            self.transactions.append(transaction)
...
class Client:
    ...
    def compose(self, message: str, recipient_name: str):
        ...
        email = Email(msg=message, sender=self, recipient_name=recipient_name)
