...
class Account:
    ...
    def time_to_retire(self, amount: float) -> int:
        ...
        return math.ceil(math.log(amount / self.balance, self.interest + 1))
        "*** YOUR CODE HERE ***"
...
class FreeChecking(Account):
    ...
    def withdraw(self, amount: int) -> int | str:
        ...
        charge = amount + (0 if self.free_withdrawals > 0 else self.withdraw_fee)
        self.free_withdrawals = max(0, self.free_withdrawals - 1)
...
def without(s: Link, i: int) -> Link:
    ...
    """
    "*** YOUR CODE HERE ***"
    if i > 0 and s.rest is Link.empty:
        return s
