...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        evaluated_args = map_link(lambda expr: scheme_eval(expr, env), rest)
...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        # BEGIN PROBLEM 9
        formals, body = procedure.formals, procedure.body
        child_frame = procedure.env.make_child_frame(formals, args)
        return eval_all(body, child_frame)
        # END PROBLEM 9
    elif isinstance(procedure, MuProcedure):
        # BEGIN PROBLEM 11
        formals, body = procedure.formals, procedure.body
        child_frame = env.make_child_frame(formals, args)
        return eval_all(body, child_frame)
        # END PROBLEM 11
    else:
