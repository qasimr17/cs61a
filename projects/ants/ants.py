...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive, min=min_range, max=max_range):
        ...
        current_place, index  = self.place, 0
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, beehive, min=min_range, max=max_range):
        return super().nearest_bee(beehive, min, max)
    # END Problem 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        self.armor -= amount 
        total_damage = self.damage + amount if self.armor <= 0 else amount
        bees = self.place.bees[:] # create a copy to iterate over
        for bee in bees:
