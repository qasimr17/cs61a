...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self) -> Bee | None:
        ...
        while not curr_place.is_hive:
            if (
                self.lower_bound <= distance_from_origin <= self.upper_bound
                and curr_place.bees
            ):
...
class FireAnt(Ant):
    ...
    def reduce_health(self, damage_taken: float):
        ...
        )  # reduce own health by damage taken

        reflective_damage = damage_taken + (self.damage if self.health <= 0 else 0)

        for bee in curr_place.bees.copy():  # iterate over copy of list
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate: GameState):
        """Eats a random bee in the current place whole."""
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate: GameState):
        # BEGIN Problem 8a
        if isinstance(self.ant_contained, Ant):
            self.ant_contained.action(gamestate=gamestate)
        # END Problem 8a
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate: GameState):
        ...
        # BEGIN Problem 12
        super().action(gamestate)
        # double the damage of each ant behind me
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self) -> dict[Bee, int]:
        ...
        while curr_place is not None and not curr_place.is_hive:
            # ant in place
            if curr_place.ant is not None and curr_place.ant is not self:
                d[curr_place.ant] = distance
                if (
                    isinstance(curr_place.ant, ContainerAnt)
                    and curr_place is not self.place
                ):
                    d[curr_place.ant.ant_contained] = distance
            # bees in place
...
class Bee(Insect):
    ...
    def blocked(self) -> bool:
        ...
        return (
            ...
            and self.place.ant.blocks_path is True
        )
    ...
    def action(self, gamestate: GameState):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        blocked = self.blocked() and self.place and self.place.ant
