...
def roll_dice(num_rolls, dice=six_sided):
    ...
    return 1 if rolled_a_one else total
    # END PROBLEM 1
...
def boar_brawl(player_score, opponent_score):
    ...
    return max(1, 3 * abs(opponent_tens_digit - player_ones_digit))
    # END PROBLEM 2
...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    """
    # BEGIN PROBLEM 4
    score_after_simple_update = simple_update(
        num_rolls, player_score, opponent_score, dice
    )
    return sus_points(score_after_simple_update)
    # END PROBLEM 4
...
def play(strategy0, strategy1, update, score0=0, score1=0, dice=six_sided, goal=GOAL):
    ...
    while scores[0] < goal and scores[1] < goal:
        # scores
        player_score = scores[who]
        opponent_score = scores[1 - who]

        # num of dice to roll
        num_rolls = strategies[who](player_score, opponent_score)
...
def is_always_roll(strategy, goal=GOAL):
    ...
    """
    # BEGIN PROBLEM 7
    base_score = strategy(0, 0)

    return all(strategy(x, y) == base_score for x in range(goal) for y in range(goal))
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    """
    # BEGIN PROBLEM 9
    # call the higher order function only once
    averaged_roll_dice = make_averaged(
        original_function=roll_dice, times_called=times_called
    )

    scores = [averaged_roll_dice(i, dice) for i in range(1, 11)]
...
def boar_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    return (
        0
        if boar_brawl(player_score=score, opponent_score=opponent_score) >= threshold
        else num_rolls
    )
    # END PROBLEM 10
